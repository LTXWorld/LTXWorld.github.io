...
如果此时打印s1与s2的内容，会是什么呢？请思考一炷香的时间。
...
切片，本身是个结构体，包含着指向**底层数组的指针，长度，容量这三个属性**；在函数值传递的过程中，在函数内部操作的是切片的副本，与原来的切片结构体地址不同，但是只要不发生扩容，底层的数组指针是相同的，如果想要相同切片，需要传入其地址；`append`操作不影响`&path`即切片结构体自身的地址,在栈中（因为其长度总会发生变化），与上面相同，**只要不发生扩容**，底层数组指针就是相同的。
